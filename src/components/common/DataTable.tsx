      ...
      if (tableName == "TrakingModels") {
        try {
          ...
          const response = await axios.get(
            `/datatable/filter/dashboard/${modelDemoNumber}/${stage}`
          );
        ...
        try {
          const response = await axios.post(`/datatable/filter/models`, {
        ...
        try {
          const response = await axios.post(`/datatable/filter/${tableName}`, {
            filters: formattedFilters,
          });
    ...
    if (fieldFilter && tableName) {
      try {
        ...
        const formattedFields = allFields.map((field) => {
            const fieldName = field.relatedTable
              ? `${field.relatedTable}.${field.column_name}`
              : field.column_name;
              
            const displayName = Object.keys(fieldFilter).find(
              (key) => fieldFilter[key] === fieldName
            );
      ...
      <div className="flex items-center justify-between mb-4">
            ...
            <div className="w-20">
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  className="bg-red-500 text-white hover:bg-red-600 rounded-md px-4 py-2 mt-4"
                >
                  {t("clear all Filters")}
                </Button>
              </div>
        ...
        <Tooltip.Provider>
          <Tooltip.Root>
            ...
            <Tooltip.Content className="text-sm bg-gray-700 text-white p-2 rounded-md">
              Export to Excel
            </Tooltip.Content>
          </Tooltip.Root>
        </Tooltip.Provider>
