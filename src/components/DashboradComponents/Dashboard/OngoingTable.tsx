...
import { holdModelVarinte, restartModelVarinte } from "@/services/ModelVarients.services.ts";
...
export default function OngoingTable({ works, setWorks, setSelectedSizes, setQuantityReceived }: Props) {
    ...
    const handlePause = async (item: any, reason: string) => {
        if (item.ModelVariant.Id && item.ModelVariant.Model?.Id) {
            await holdModelVarinte(setCurrentVariant, item.ModelVariant.Id, reason, item.ModelVariant.Model.Id);
    ...
    const handleRestart = async (item: any) => {
        if (item.ModelVariant.Id && item.ModelVariant.Model?.Id) {
            await restartModelVarinte(setCurrentVariant, item.ModelVariant.Id, item.ModelVariant.Model.Id);
            setActionInProgress(true);
        }
    };
        ...
        <>
            ...
            <TableCell className="space-x-1 space-y-1">
                ...
                {user?.category === "CUTTING" ? (
                    <Button
                        onClick={() => handleSendCuttingConfirmation(item)}
                    >
